	...
	public function refresh() {
		...
		if ($this->validate()) {
			...
			foreach ($xml as $xml) {
				...
				$error_log_file = array();
				$error_log_file[] = '----------------------------------------------------------------';
				$error_log_file[] = 'MOD: ' . $dom->getElementsByTagName('name')->item(0)->textContent;
...
$error_log_file[] = 'REGEX: ' . $search;
	...
	protected function getList() {
		...
		foreach($modified_files as $modified_file) {
			$data['modified_files'][] = array(
				'file' => $modified_file,
				'by' => isset($modification_files[$modified_file]) ? $modification_files[$modified_file] : array()
			);
		}
	...
	public function getForm() {
		...
		$data['button_save_refresh'] = $this->language->get('button_save_refresh');
		...
		if (isset($this->request->get['modification_id'])) {
			...
			$data['action'] = $this->url->link('extension/modification/edit', 'token=' . $this->session->data['token'] . '&modification_id=' . $modification_info['modification_id'] . $url, 'SSL');

			$this->document->setTitle($modification_info['name'] . ' » ' . $data['heading_title']);
		} else {
			$data['text_form'] = $this->language->get('text_add');

			$data['action'] = $this->url->link('extension/modification/add', 'token=' . $this->session->data['token'] . $url, 'SSL');

			$this->document->setTitle($data['heading_title']);
		}

		$data['cancel'] = $this->url->link('extension/modification', 'token=' . $this->session->data['token'] . $url, 'SSL');
						...
						if ($files) {
							foreach ($files as $file) {
								...
								if (isset($return[$file])) {
									$return[$file][$dom->getElementsByTagName('code')->item(0)->textContent] = $dom->getElementsByTagName('name')->item(0)->textContent;
								} else {
									$return[$file] = array(
										$dom->getElementsByTagName('code')->item(0)->textContent => $dom->getElementsByTagName('name')->item(0)->textContent
									);
								}
							}
						}
