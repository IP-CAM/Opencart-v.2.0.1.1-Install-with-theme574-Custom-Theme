...
class ControllerModulePDQO extends Controller {
	public function index() {
        ...
        $data = array();
        $data['t'] = array_merge($data, $this->language->load('module/pdqo'));
		...
		if(isset($settings['pdqo_module']['settings'])) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/pdqo/default.tpl')) {
				$tpl = $this->load->view($this->config->get('config_template') . '/template/module/pdqo/default.tpl', $data);
			} else {
				$tpl = $this->load->view('default/template/module/pdqo/default.tpl', $data);
			}
	...
	public function get_cart() {
        ...
        foreach($cart as $product) {
            if(isset($this->request->post['key']) && $this->request->post['key'] != $product['key']) continue;
