	...
	public function index() {
		$this->language->load('module/newfastorder');	
		  ...
		  $data['tax_rates'] = $this->tax->getRates(0, $product_info['tax_class_id']);
			...
			foreach ($this->model_catalog_product->getProductOptions($product_id) as $option) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$oid = $option['product_option_id'];


						//echo $option_value['option_value_id'] . '====';
						if($option['option_id'] == 13 && isset($_GET['option'][$oid]) && $_GET['option'][$oid] == $option_value['product_option_value_id'] && $option_value['image'] ){
							$data['thumb'] = $this->model_tool_image->resize($option_value['image'], 228, 228);
						}
						
						if($option['option_id'] == 15 && isset($_GET['option'][$oid]) && $_GET['option'][$oid] == $option_value['product_option_value_id'] && $option_value['product_option_value_id'] != 166){
							$data['tara'] = 1;
						}
						
						if($option['option_id'] == 17 && isset($_GET['option'][$oid]) && $_GET['option'][$oid] == $option_value['product_option_value_id']){
							$data['fltr'] = 1;

                            if($_special) {
							$_special += $option_value['price'];
                            $data['special'] = $this->currency->format($this->tax->calculate($_special, $product_info['tax_class_id'], $this->config->get('config_tax')));
						   $data['special_value'] = $_special;

							$data['fltr_name'] = $option['name'];
							}


							$_price += $option_value['price'];

							$data['price']  = $this->currency->format($this->tax->calculate($_price, $product_info['tax_class_id'], $this->config->get('config_tax')));
						    $data['price_value'] = $_price;
			...
			/*if(isset($_GET['option'][333]) && $_GET['option'][333]){
				$product_info33 = $this->model_catalog_product->getProduct($_GET['option'][333]);
				if ($product_info33['image']) {
					$data['popup'] = $this->model_tool_image->resize($product_info33['image'], 228, 228);
				} else {
					$data['popup'] = $this->model_tool_image->resize('no_image.jpg', 228, 228);
				}
			}*/
			...
			$data['withOptions'] = $data['options'] ? true : false;
