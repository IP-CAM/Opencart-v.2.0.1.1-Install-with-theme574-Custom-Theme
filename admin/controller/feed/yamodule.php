...
class ControllerFeedYamodule extends Controller {
	...
	public function initForm($array)
	{
		...
		$data['ya_kassa_check'] = str_replace("http://","https://",HTTPS_CATALOG).'payment/yamodule/callback';
		$data['ya_kassa_aviso'] = str_replace("http://","https://",HTTPS_CATALOG).'payment/yamodule/callback';
		$data['ya_pokupki_sapi'] = HTTPS_CATALOG.'yandexbuy';
		if ($this->config->get('config_secure'))
		{
			$data['ya_kassa_fail'] = HTTPS_CATALOG.'checkout/failure';
			$data['ya_kassa_success'] = HTTPS_CATALOG.'checkout/success';
			$data['ya_p2p_linkapp'] = HTTPS_CATALOG.'payment/yamodule/inside';
			$data['ya_market_lnk_yml'] = HTTPS_CATALOG.'feed/yamarket';
		}
		else
		{
			$data['ya_kassa_fail'] = HTTP_CATALOG.'checkout/failure';
			$data['ya_kassa_success'] = HTTP_CATALOG.'checkout/success';
			$data['ya_p2p_linkapp'] = HTTP_CATALOG.'payment/yamodule/inside';
			$data['ya_market_lnk_yml'] = HTTP_CATALOG.'feed/yamarket';
		}
