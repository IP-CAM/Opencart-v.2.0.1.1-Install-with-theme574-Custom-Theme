...
class ControllerModuleMicrodataPro extends Controller {
	...
	public function index() {
		...
		if($this->microdatapro->opencart_version(1) >= 3 && $this->microdatapro->opencart_version(1) == 2){ //IF OC > 2.3
			$data = $this->language->load('extension/module/microdatapro');
		}else{
			$data = $this->language->load('module/microdatapro');
		}
		...
		$data['action'] = $this->url->link('module/microdatapro', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$data['modules_link'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
